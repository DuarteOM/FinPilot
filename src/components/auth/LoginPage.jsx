...
export default function LoginPage({T,dark,loading,onLogin,onGoogle,onRegister}){
  ...
  const submit=async()=>{
    setErr("");
    try{
      if(tab==="login"){if(!email||!pw){setErr("Preenche todos os campos.");return;}await onLogin({email,password:pw});}
      else{if(!name||!email||!pw||!pw2){setErr("Preenche todos os campos.");return;}if(pw!==pw2){setErr("As palavras-passe não coincidem.");return;}await onRegister({name,email,password:pw});}
  ...
  return(
    <div style={{fontFamily:"'Inter',-apple-system,sans-serif",background:T.bg,color:T.text,minHeight:600,borderRadius:20,border:`1px solid ${T.border}`,overflow:"hidden",display:"flex",alignItems:"stretch"}}>
      ...
      <div style={{flex:1.1,background:`linear-gradient(155deg,${T.panel2},${T.bg})`,display:"flex",flexDirection:"column",justifyContent:"space-between",padding:40,position:"relative",overflow:"hidden",borderRight:`1px solid ${T.border}`}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:7,fontSize:11.5,color:T.sub,position:"relative",zIndex:1}}><ShieldCheck size={14} color={T.accent}/>Palavras-passe protegidas com scrypt · Privacidade desde a conceção</div>
      </div>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:40}}>
        <div style={{width:300,animation:"fade-up .5s cubic-bezier(.2,.7,.3,1) both"}}>
          ...
          <div className="fp-btn" onClick={!loading?google:undefined} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"11px",borderRadius:10,border:`1px solid ${T.border}`,background:dark?T.panel2:"#FFF",fontSize:13.5,fontWeight:500,color:T.text,marginBottom:14}}>
            {loading?<><div className="spinner" style={{width:16,height:16,borderRadius:"50%",border:`2.5px solid ${T.border}`,borderTopColor:T.accent}}/>A ligar à conta Google…</>:<><GoogleLogo size={17}/>Continuar com Google</>}
          </div>
          ...
          <div className="fp-btn" onClick={!loading?submit:undefined} style={{marginTop:14,textAlign:"center",padding:"12px",borderRadius:10,background:T.accent,color:"#0A0D12",fontSize:13.5,fontWeight:600}}>
