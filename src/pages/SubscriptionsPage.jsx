...
export default function PageSubscriptions({T,subs,setSubs,setModal,toast}){
  ...
  const restore=async id=>{try{await api.subscriptions.setActive(id,true);setSubs(ss=>ss.map(s=>s.id===id?{...s,active:true}:s));toast("success","Subscrição reativada.");}catch(error){toast("error",error.message);}};
