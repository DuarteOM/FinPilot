...
export default function PageTransactions({T,txs,setTxs,setModal,toast}){
  ...
  const delTx=async id=>{try{await api.transactions.remove(id);setTxs(ts=>ts.filter(t=>t.id!==id));toast("info","Transação eliminada.");}catch(error){toast("error",error.message);}};
