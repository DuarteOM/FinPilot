    ...
    try {
      ...
      setMsgs(m => {
        ...
        if (last?.role === "assistant" && last.text === "")
          copy[copy.length - 1] = { ...last, text: error.message || "Ocorreu um erro. Tenta novamente." };
        return copy;
      });
    } finally {
      setTyping(false);
    ...
    <div className="slide-r" style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: 330, background: T.panel, borderLeft: `1px solid ${T.border}`, display: "flex", flexDirection: "column", zIndex: 10 }}>
      ...
      <div style={{ padding: "14px 16px 12px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {msgs.length > 1 && (
            <div className="fp-btn" onClick={clearHistory} title="Apagar histórico" style={{ padding: 4 }}>
              <Trash2 size={14} color={T.mut} />
            </div>
          )}
          <div className="fp-btn" onClick={onClose}><X size={15} color={T.sub} /></div>
        </div>
      </div>
      ...
      <div className="fp-scroll" style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
        {msgs.map((m, i) => (
          <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "85%", padding: "9px 12px", borderRadius: 12, fontSize: 12.5, lineHeight: 1.55, background: m.role === "user" ? T.accent : T.panel2, color: m.role === "user" ? "#0A0D12" : T.text, border: m.role === "assistant" ? `1px solid ${T.border}` : "none", animation: "fade-up .22s ease both", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
            {m.text}
          </div>
        ))}
        {typing && msgs[msgs.length - 1]?.text === "" && (
          <div style={{ display: "flex", gap: 4, padding: "9px 12px" }}>
            {[0, 1, 2].map(d => <span key={d} className="dot" style={{ width: 5, height: 5, borderRadius: "50%", background: T.mut, animationDelay: `${d * 0.15}s` }} />)}
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      ...
      <div style={{ padding: 12, borderTop: `1px solid ${T.border}`, display: "flex", gap: 8 }}>
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          placeholder="Pergunta algo sobre as tuas finanças…" disabled={typing}
          style={{ flex: 1, background: T.panel2, border: `1px solid ${T.border}`, borderRadius: 9, padding: "9px 11px", fontSize: 12, color: T.text, outline: "none", opacity: typing ? 0.6 : 1 }} />
        <div className="fp-btn" onClick={() => send(input)}
          style={{ width: 34, height: 34, borderRadius: 9, background: T.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, opacity: typing || !input.trim() ? 0.5 : 1 }}>
