...
export default function TopBar({ T, view, profile, notifs, setNotifs, searchQ, setSearchQ, searchOpen, setSearchOpen, notifOpen, setNotifOpen, searchResults }) {
  ...
  return (
    <div style={{ padding: "18px 28px 0", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
      {/* Title */}
      <div>
        <div className="fp-disp" style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 19, fontWeight: 600, letterSpacing: -0.3 }}>
          {PAGE_TITLES[view] ?? view}
        </div>
        <div style={{ fontSize: 12, color: T.sub, marginTop: 2 }}>
          {new Intl.DateTimeFormat("pt-PT", { weekday: "long", day: "numeric", month: "long", year: "numeric" }).format(new Date())}
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                : searchResults.map(t => (
                  <div key={t.id} className="fp-row" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 8px", borderRadius: 8, cursor: "pointer" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                      <div style={{ width: 26, height: 26, borderRadius: 8, background: `${t.color}22`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <t.icon size={12} color={t.color} />
                      </div>
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 500 }}>{t.merchant}</div>
                        <div style={{ fontSize: 10.5, color: T.sub }}>{t.cat} · {t.ds}</div>
                      </div>
                    </div>
                    <span className="fp-num" style={{ fontSize: 12, fontWeight: 600, color: t.amount > 0 ? T.accent : T.text }}>
        ...
        <div ref={notifRef} style={{ position: "relative" }}>
          <div className="fp-btn" onClick={() => setNotifOpen(o => !o)}
            style={{ width: 32, height: 32, borderRadius: 9, border: `1px solid ${notifOpen ? T.accent : T.border}`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", transition: "border-color .2s" }}>
            <Bell size={14} />
          ...
          {notifOpen && (
            <div className="scale-in" style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, width: 320, background: T.panel, border: `1px solid ${T.border}`, borderRadius: 12, boxShadow: "0 12px 28px rgba(0,0,0,.22)", zIndex: 30, transformOrigin: "top right", overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", borderBottom: `1px solid ${T.border}` }}>
                ...
                {unread > 0 && (
                  <div className="fp-btn" onClick={() => setNotifs(ns => ns.map(n => ({ ...n, read: true })))}
                    style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10.5, color: T.sub }}>
